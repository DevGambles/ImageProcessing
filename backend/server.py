...
raw_image = np.load('./weights/raw_image.npy')#[:, :, :, 0]  # Candidates, i.e. DeepLesion test images
...
def index():
    if request.method == 'POST':
        ...
        return render_template(
            'index.html',
            query_path=prepare_for_render(query),
...
if __name__=="__main__":
    app.run("localhost", port=44442, debug=True)
